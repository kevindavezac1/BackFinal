...
const asignarTurnoPaciente = async (req, res) => {
  try {
      ...
      const { nota, id_medico, fecha, hora, id_paciente, id_cobertura } = req.body;
      console.log('Datos recibidos:', { nota, id_medico, fecha, hora, id_paciente, id_cobertura }); // Log para depuración

      const turno = { nota, id_medico, fecha, hora, id_paciente, id_cobertura };
      const connection = await getConnection();
      await connection.query("INSERT INTO turno SET ?", turno);
      res.json({ codigo: 200, message: "Turno asignado correctamente", payload: [] });
  } catch (error) {
      console.error('Error al asignar turno:', error); // Log para depuración
